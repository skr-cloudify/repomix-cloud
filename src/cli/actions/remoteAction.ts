...
export const cloneRepository = async (
  ...
  logger.log(
    `Clone repository: ${url} to temporary directory. ${pc.dim(
      `path: ${directory}`
    )}`
  );
  logger.log("");
  ...
  try {
    await deps.execGitShallowClone(url, directory, remoteBranch, gitHubToken);
  } catch (error) {
    throw new RepomixError(
