...
import {
  type ParseStrategy,
  createParseStrategy,
} from "./parseStrategies/ParseStrategy.js";

// Lazy import to avoid import.meta.url issues in Smithery
let loadLanguageModule: any = null;
const getLoadLanguage = async () => {
  if (!loadLanguageModule) {
    loadLanguageModule = await import("./loadLanguage.js");
  }
...
export class LanguageParser {
  ...
  private async prepareLang(name: SupportedLang): Promise<LanguageResources> {
    ...
    try {
      const loadLanguage = await getLoadLanguage();
  ...
  private async getResources(name: SupportedLang): Promise<LanguageResources> {
    if (!this.initialized) {
      throw new RepomixError(
        "LanguageParser is not initialized. Call init() first."
      );
    }
